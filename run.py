...
while (request_count < file_len):
    ...
    ### Output Daily Weather tables in multiple Excel with Postal Code as file name ###
    df = stuff1.set_index('time')
    ##test output to csv for each postal code
    #df.to_csv(str(readfile.postal_code[request_count])+'.csv')
    ####################################################
